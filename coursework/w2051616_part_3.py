...
def save_data(List,listname):      #a function to save progression data to a file
    with open("progression_data.txt",'a') as file:     #open the file and then append each element into that file
        for i in List :
            file.write(f"{listname}-" + str(i[0])+','+str(i[1])+','+str(i[2])+'\n')     #use string formatting method to display
        ...
        while True:
            ...
            if total_credits == 120:     #checking if total credits are equal to 120
                ...
                if Pass == 120:
                    ...
                    Progress_list.append([Pass , Defer , Fail])     #Add the credits to progress list
                    Progress += 1
                elif Pass == 100 :
                    print("PROGRESS(MODULE TRAILER)")     #Display the prediction of the student's progress
                    Progress_mt_list.append([Pass , Defer , Fail])     #Add the credits to progress (module trailer)list
                    Progress_mt += 1 
                elif Fail >= 80:
                    print("EXCLUDE")     #Display the prediction of the student's progress
                    Exclude_list.append([Pass , Defer , Fail])     #Add the results to exclude list
                    Exclude += 1
                else:
                    print("DO NOT PROGRESS-MODULE RETRIEVER")     #Display the prediction of the student's progress
                    Module_ret_list.append([Pass , Defer , Fail])     #Add the results to module retriever list
                    Module_ret += 1
                print ("---------------------------------------------------------------------------------------")
                ...
                choice = input("\n Would you like to enter another set of data?(enter 'y' for yes or 'q' for quite ) :" )   #input y or q to continue
                break
    elif choice == 'q':     #stop adding data to the programme
...
file = open("progression_data.txt",'w')     #delete old data which are saved in the progression file
file.close()

save_data(Progress_list, "Progress")     #calling the function to save data in progress section
save_data(Progress_mt_list, "Progress(module trailer)")     #calling the function to save data in module trailer section
save_data(Module_ret_list, "Module retriever")    #calling the function to save data in module retriever section
save_data(Exclude_list, "Exclude")      #calling the function to save data in exclude section

with open("progression_data.txt",'r') as file:     #read and output the progression data from the file
    print(file.read())
